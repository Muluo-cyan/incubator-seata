 ...
 *
 */
public class ConsoleAuthenticationTokenFilter extends OncePerRequestFilter {
     ...
     */
    public ConsoleAuthenticationTokenFilter(JwtTokenUtils tokenProvider) {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
        ...
        if (accessToken != null) {
            ...
            if (result.getMessage().equals(Code.ACCESS_TOKEN_NEAR_EXPIRATION.getMsg())) {
                ...
                response.addHeader(ConsoleSecurityConfig.ACCESS_TOKEN_NEAR_EXPIRATION, "true");
            }
    ...
    private String resolveAccessToken(HttpServletRequest request) {
        String bearerToken = request.getHeader(ConsoleSecurityConfig.AUTHORIZATION_HEADER);
        if (bearerToken != null && bearerToken.startsWith(ConsoleSecurityConfig.TOKEN_PREFIX)) {
            String accessToken = bearerToken.substring(ConsoleSecurityConfig.TOKEN_PREFIX.length());
            return StringUtils.hasText(accessToken) ? accessToken : null;
        }
        String accessToken = request.getParameter(ConsoleSecurityConfig.AUTHORIZATION_TOKEN);
    ...
    private String resolveRefreshToken(HttpServletRequest request) {
        String refreshToken = request.getHeader(ConsoleSecurityConfig.REFRESH_TOKEN);
